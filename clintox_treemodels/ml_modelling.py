...
OUTPUT_DIR = "/home/raghvendra2/Molformer_Finetuning/classification_model_clintox/Results"
...
def train_and_evaluate(X_train, y_train, X_val, y_val, X_test, y_test, best_params, embedding_name):
    ...
    # Get probabilities for positive class if binary classification
    y_proba_xgb = xgb_model.predict_proba(X_test)
    if y_proba_xgb.shape[1] == 2:  # Binary classification
        y_proba_xgb = y_proba_xgb[:, 1]  # Probability of positive class
    ...
    # AUC calculation based on classification type
    if len(np.unique(y_test)) <= 2:  # Binary classification
        xgb_auc = round(roc_auc_score(y_test, y_proba_xgb), 4)
    else:  # Multi-class
    ...
    if y_proba_lgb.shape[1] == 2:  # Binary classification
        y_proba_lgb = y_proba_lgb[:, 1]  # Probability of positive class
    ...
    # AUC calculation based on classification type
    if len(np.unique(y_test)) <= 2:  # Binary classification
        lgb_auc = round(roc_auc_score(y_test, y_proba_lgb), 4)
    else:  # Multi-class
    ...
    if y_proba_cat.shape[1] == 2:  # Binary classification
        y_proba_cat = y_proba_cat[:, 1]  # Probability of positive class
    ...
    # AUC calculation based on classification type
    if len(np.unique(y_test)) <= 2:  # Binary classification
        cat_auc = round(roc_auc_score(y_test, y_proba_cat), 4)
    else:  # Multi-class
