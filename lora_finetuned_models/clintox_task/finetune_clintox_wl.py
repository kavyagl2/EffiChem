...
def data_load():
    train_clintox=pd.read_csv('clintox_datasets/clintox_train.csv')
    val_clintox=pd.read_csv('clintox_datasets/clintox_valid.csv')

    return train_clintox, val_clintox
...
def data_prep(data_process,tokenizer):
    ...
    labels = data_process['FDA_APPROVED'].tolist() 
...
sweep_config = {
"name": "clintox_Hyperparameter_Tuning",
"method": "bayes",
"metric": {
    "goal": "maximize", 
...
for model_name in model_list:
    ...
    def run_training():
        ...
        run = wandb.init(project="clintox_Hyperparameter_Tuning")
        config = run.config
        ...
        save_dir = f"./WL_Loss/{model_id_clean}/{run_id}"
        logging_dir = f"./WL_Loss/{model_id_clean}/{run_id}"
...
#Load the test data
test_data=pd.read_csv('clintox_datasets/clintox_test.csv')

#Path to model root
models_root_dir = "./WL_Loss/" #add model saved path
output_dir = "./test_results_clintox/"
...
for key in MODEL_NAME_MAP.keys():
    ...
    test_labels = test_data['FDA_APPROVED'].tolist() 
    ...
    save_path = "./weighted_loss_clintox/"+key+"_LoRA_Finetuned/"
...
all_test_results_df.columns = ["MCC","LOSS","Time","ACC","AUROC","PREC","REC","F1","F1-MICRO","RUNTIME","SAMPLE_PER_SECOND","STEPS_PER_SECOND","BEST_MODEL"]
