...
for model_name in model_list:
    ...
    def run_training():
        ...
        trainer_flavor = FocalLossTrainer(
            model=lora_model,
            args=training_args,
            train_dataset=training_data,
            eval_dataset=validation_data,
...
for key in MODEL_NAME_MAP.keys():
    ...
    save_path = "./focal_loss_flavor/"+key+"_LoRA_Finetuned/"
    final_adapter_model.save_pretrained(save_path)

    del trainer, adapter_model, base_model, final_adapter_model, test_dataset, tokenizer
    torch.cuda.empty_cache()
